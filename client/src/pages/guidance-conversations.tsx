 ...
 * Each thread shows a preview and can be expanded inline to read the full exchange.
 *
...
interface LearningThreadFull extends LearningThreadSummary {
...
function ThreadCard({
  ...
  const { data: fullThread, isFetching } = useQuery<LearningThreadFull>({
    queryKey: ["/api/learning-threads", thread.id],
    queryFn: async () => {
      const res = await apiRequest("GET", `/api/learning-threads/${thread.id}`);
      const data = (await res.json()) as { thread: LearningThreadFull };
      return data.thread;
    },
    enabled: expanded,
    staleTime: 300_000,
  ...
  return (
    ...
    >
      <CardContent className="p-4 space-y-2">
        ...
        {expanded && (
          <ScrollArea className="max-h-72 rounded-xl bg-muted/40 p-3 space-y-2 mt-2">
            {isFetching && (
              <div className="space-y-2">
                {[1, 2, 3].map((i) => (
                  <div key={i} className={`h-8 rounded-lg bg-muted animate-pulse ${i % 2 === 0 ? "ml-6" : "mr-6"}`} />
                ))}
              </div>
            )}
            {!isFetching && fullThread?.messages?.map((msg, i) => (
              <div
                key={i}
                className={`text-xs leading-relaxed px-3 py-2 rounded-xl max-w-[88%] ${
                  msg.role === "user"
        ...
        <div className="flex justify-end">
          <Button
            ...
            data-testid={`btn-expand-thread-${thread.id}`}
          ...
          >
            {expanded ? (
              ...
              <>
                Read thread <ChevronDown className="h-3 w-3" />
              </>
            )}
          </Button>
        </div>
