...
/** Format an ElevationPlanFull as a plain-text summary for copy/share */
function formatPlanAsText(planData: ElevationPlanFull): string {
  const { plan, days } = planData;
  const lines: string[] = [];
  lines.push(`📋 ${plan.title}`);
  if (plan.goal) lines.push(`🎯 Goal: ${plan.goal}`);
  if (plan.focusDimension) lines.push(`🔎 Focus: ${plan.focusDimension}`);
  lines.push(`📅 ${plan.startDate} → ${plan.endDate}`);
  lines.push(`Status: ${plan.status}`);
  lines.push("");

  for (const day of days) {
    lines.push(`─── Day ${day.dayIndex}: ${day.theme} ───`);
    lines.push(`Intention: ${day.intention}`);
    for (const action of day.actions) {
      const done = action.isCompleted ? "✅" : "⬜";
      lines.push(`  ${done} [${action.actionType}] ${action.title}`);
      if (action.description) lines.push(`       ${action.description}`);
    }
    lines.push("");
  }

  lines.push("Generated by DW.ai — Dimensional Wellness AI");
  return lines.join("\n");
}
          ...
          <TabsContent value="plan" className="mt-4 space-y-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {shareEnabled && (
                        <Button
                          ...
                          aria-label="Share or export plan"
            ...
            {shareEnabled && (
              <ShareExportSheet
                ...
                title={plan.title}
                description="Share or export your 7-day elevation plan"
                shareUrl={shareUrl}
                textContent={formatPlanAsText(planData)}
              />
            )}
