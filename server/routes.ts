...
Response:`;
  ...
  app.post("/api/life-system/import/parse", requireAuth, async (req, res) => {
    try {
      ...
      console.log(`[dw-import] parsing ${trimmed.length} chars with rule-based parser`);

      // ── Primary: rule-based parser (works without AI) ─────────────────────
      const rulesParsed = parseLifeSystemRuleBased(trimmed);
      console.log(`[dw-import] rule-based detected: ${rulesParsed.detectedTypes.join(", ")}, goals=${rulesParsed.goals.length}, days=${Object.keys(rulesParsed.weeklySchedule).length}`);

      // ── Optional: AI enhancement pass (if proxy is healthy) ──────────────
      // Try a short probe; if it fails, skip AI and return the rule-based result.
      let aiEnhanced = false;
      let aiMeta: any = {};
      let aiScheduleExtra: Record<string, any> = {};
      let journalEntries: any[] = [];
      let affirmations: any[] = [];
      let readingList: any[] = [];
      let financialGoals: any[] = [];
...
title = a short descriptive title for the content (e.g. "My Weekly Routine", "March Journal", "Reading List 2024").`,
          ...
          const aiTypes: string[] = Array.isArray(detection?.types) ? detection.types : rulesParsed.detectedTypes;
          const aiTitle: string = detection?.title || rulesParsed.rawTitle;
          console.log(`[dw-import] AI detected types: ${aiTypes.join(", ")}`);

          // Merge AI-detected types with rules-detected
          const mergedTypes = [...new Set([...rulesParsed.detectedTypes, ...aiTypes])];

          const isLifeSystem = aiTypes.some(t => ["life_system", "workout_plan", "meal_plan", "grocery_list"].includes(t));
          const hasGoals = aiTypes.some(t => ["life_system", "goals", "financial_plan", "project_plan"].includes(t));
          const isJournal = aiTypes.includes("journal_entry");
          const isAffirmations = aiTypes.includes("affirmations");
          const isReading = aiTypes.includes("reading_list");
          const isFinancial = aiTypes.includes("financial_plan");
          const isProject = aiTypes.includes("project_plan");
          const isNotes = aiTypes.includes("notes") && aiTypes.length === 1;

          // Build extraction calls (only for types not already handled well by rule-based)
...
content = the full journal text (preserve the writing). tags = 1-4 theme tags.`,
              trimmed.slice(0, CHUNK_MAX)
            ));
            labels.push("journal");
          }
          ...
          if (isAffirmations) {
            calls.push(callAI(
              `Extract affirmations/mantras. Return ONLY valid JSON: {"affirmations": [string]}. Keep exact wording.`,
              trimmed.slice(0, CHUNK_MAX)
            ));
            labels.push("affirmations");
          }

          if (isReading) {
            calls.push(callAI(
              `Extract reading/watch/listen list items. Return ONLY valid JSON: {"readingList": [{title, author, type, notes}]}.
type is one of: book, article, video, podcast, course.`,
              trimmed.slice(0, CHUNK_MAX)
            ));
            labels.push("reading");
          }
...
notes = full text preserved. title = short title. tags = 1-3 theme tags.`,
              trimmed.slice(0, CHUNK_MAX)
            ));
            labels.push("notes");
          }
          ...
          if (calls.length > 0) {
            ...
            results.forEach((r, idx) => {
              ...
              if (label === "meta") aiMeta = r || {};
              else if (label === "journal") journalEntries = r?.journalEntries || [];
              else if (label === "affirmations") affirmations = r?.affirmations || [];
              else if (label === "reading") readingList = r?.readingList || [];
              else if (label === "financial") financialGoals = r?.financialGoals || [];
              else if (label === "project") projectTasks = r?.projectTasks || [];
              else if (label === "notes") { notesText = r?.notes || ""; notesTags = r?.tags || []; }
            });
          }

          aiEnhanced = true;
          rulesParsed.rawTitle = aiTitle || rulesParsed.rawTitle;
          rulesParsed.detectedTypes = mergedTypes;
          console.log(`[dw-import] AI enhancement succeeded`);
        } catch (aiErr: any) {
          console.warn(`[dw-import] AI enhancement skipped (${aiErr?.status ?? aiErr?.message ?? "error"}), using rule-based result only`);
        }
      }

      // ── Merge rule-based + AI results ────────────────────────────────────
      // AI meta overrides rule-based where AI found richer data
      const finalGoals = (aiMeta.goals && aiMeta.goals.length > 0) ? aiMeta.goals : rulesParsed.goals;
      const finalCoreRules = (aiMeta.coreRules && aiMeta.coreRules.length > 0) ? aiMeta.coreRules : rulesParsed.coreRules;
      const finalMorning = aiMeta.morningRoutine ?? rulesParsed.morningRoutine;
      const finalWindDown = aiMeta.windDownRoutine ?? rulesParsed.windDownRoutine;
      const finalGrocery = aiMeta.groceryList ?? rulesParsed.groceryList;
      const finalMealPrep = (aiMeta.mealPrepItems && aiMeta.mealPrepItems.length > 0) ? aiMeta.mealPrepItems : rulesParsed.mealPrepItems;

      // Schedule: rule-based is primary (more reliable); AI can fill gaps
      const finalSchedule: Record<string, any> = { ...rulesParsed.weeklySchedule };
      if (aiScheduleExtra) {
        for (const [day, dayData] of Object.entries(aiScheduleExtra)) {
          if (finalSchedule[day]) {
            if (!finalSchedule[day].workout && (dayData as any).workout) finalSchedule[day].workout = (dayData as any).workout;
            if (!finalSchedule[day].appWork && (dayData as any).appWork) finalSchedule[day].appWork = (dayData as any).appWork;
          }
        }
      }

      const parsed: any = {
        rawTitle: rulesParsed.rawTitle,
        detectedTypes: rulesParsed.detectedTypes,
        goals: finalGoals,
        coreRules: finalCoreRules,
        morningRoutine: finalMorning,
        windDownRoutine: finalWindDown,
        weeklySchedule: finalSchedule,
        groceryList: finalGrocery,
        mealPrepItems: finalMealPrep,
        journalEntries,
        affirmations,
        readingList,
        financialGoals,
        ...
        notes: notesText,
        notesTags,
        _parsedWith: aiEnhanced ? "rules+ai" : "rules",
      };
