...
import {
  ...
  insertAiSyncSessionSchema,
  insertAiSyncItemSchema,
  insertInteractionEventSchema,
  // Strict, partial *UpdateSchema exports added by Task #124. Each one omits
  // the owner key (userId or parent FK) and rejects unknown body fields so
  // a client can no longer smuggle extra columns into a PATCH/PUT.
...
/**
 * Format a Zod validation error into a friendly 400 response payload.
 * Picks the first issue and includes the field path so clients can highlight it.
 */
...
export async function registerRoutes(
  ...
  // Voice-onboarding structured extraction, profile, and suggestion acceptance
  ...
  app.post("/api/sync/sessions", requireAuth, async (req, res) => {
    const parsed = insertAiSyncSessionSchema
      .partial()
      .safeParse(req.body ?? {});
    if (!parsed.success) {
  ...
  app.post("/api/sync/items", requireAuth, async (req, res) => {
    const arraySchema = z.array(insertAiSyncItemSchema);
    if (Array.isArray(req.body)) {
      ...
      try {
        const items = await storage.createSyncItems(parsed.data);
        res.json(items);
      } catch (error) {
    ...
    const parsed = insertAiSyncItemSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json(zodError(parsed.error));
    }
    try {
      const item = await storage.createSyncItem(parsed.data);
      res.json(item);
    } catch (error) {
      console.error("Create sync item error:", error);
      res.status(500).json({ error: "Failed to create sync item" });
    }
  });
  ...
  app.patch("/api/sync/items/:id", requireAuth, async (req, res) => {
    ...
    try {
      const updated = await storage.updateSyncItem(req.params.id, {
        ...parsed.data,
        decidedAt: parsed.data.userDecision ? new Date() : undefined,
      });
      res.json(updated);
    } catch (error) {
  ...
  app.post("/api/interactions", requireAuth, async (req, res) => {
    // Always trust the session for userId — never accept it from req.body.
    const { userId: _ignoredUserId, ...rest } = (req.body ?? {}) as Record<string, unknown>;
    const parsed = insertInteractionEventSchema
      .omit({ userId: true })
      .safeParse(rest);
    if (!parsed.success) {
      return res.status(400).json(zodError(parsed.error));
  ...
  // Both /api/chat and /api/chat/smart are implemented in
  // ./routes/chat-handlers.ts so they can be mounted into a fresh Express
...
Return ONLY the JSON array, no other text. Return 3-5 relevant results.`
  ...
  // NOTE: /api/accountability/reminders/{skip,skip-batch,restore,restore-batch}
  // moved to server/routes/accountability-routes.ts (Task #126) so every
  // /api/accountability/* handler lives in one canonical module.
...
{"resources":[{"id":"r1","title":"Resource title","description":"1-2 sentences about what you'll learn","source":"YouTube / Coursera / etc","url":"https://real.url","duration":"20 min / 4 hours / etc","type":"video or course or article or podcast","whyPicked":"1 sentence why this matters for their goals","canAddToSchedule":true}]}`;
  ...
  // NOTE: GET /api/accountability/check-in-status and
  // POST /api/accountability/evening-check-in moved to
  // server/routes/accountability-routes.ts (Task #126).
