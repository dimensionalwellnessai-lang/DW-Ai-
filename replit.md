...
*   **Life Switchboard**: Core feature implementing the "Flip the Switch" philosophy. 8 life dimensions (Body, Mind, Time, Purpose, Money, Relationships, Environment, Identity) are represented as "switches" that users can train. Each switch has status tracking (Off → Flickering → Stable → Powered), perspective training quotes, and check-in prompts. Switch data is persisted in local storage (`fts_switch_data`). Routes: `/switchboard` for the main view, `/switch/:id` for individual training pages, `/switchboard/intake` for the initial assessment flow that evaluates each dimension and sets initial switch statuses.
*   **Plan Library**: 32 curated action templates (8 switches × 4 time bands: tiny=10min, small=25min, medium=50min, large=90min). Each template includes actionNow steps, perspective text, and journalPrompt. Templates are in `client/src/config/plan-library.ts`.
*   **User Signals System**: Flag-based recommendation engine tracking energy level, stress level, time band preference, and 8 warning flags (overwhelm, timeChaos, moneyStress, relationshipDrain, envMess, lowEnergy, lowMotivation, sleepDebt). Flags have 14-day rolling counts for persistent pattern detection. Mode system (restoring vs training) adapts recommendations based on user state. Implementation: `client/src/lib/user-signals.ts`.
*   **FTS Home Page**: `/home` route shows system snapshot (energy/time selectors, recent switches), AI-recommended action card, and plan summary. Adapts between first-time splash view and returning user dashboard based on intake/onboarding completion flags.
*   **Weekly Plan Page**: `/plan` route for managing weekly training actions. Items grouped by switch, with completion tracking, time estimates, and Start/Remove actions. Data stored in `localStorage` key `fts_weekly_plan`.
*   **My Progress Page**: `/profile/progress` route showing user's sanitized wellness snapshot. Displays energy/stress/consistency metrics, switchboard status with 8 switch cards, weekly wins (completed actions count), and 14-day pattern summaries. Works for both authenticated and guest users via localStorage.
*   **Admin Analytics Page**: `/admin/analytics` route for admin-only dashboard. Shows KPIs (DAU/WAU/MAU), activation funnel metrics, and error tracking. Protected by `requireAdmin` middleware - non-admins see "Not Authorized" message.
*   **Role-Based Access Control**: Users table has `role` field (user/admin, default "user"). `requireAdmin` middleware protects admin routes. `useUserRole` hook provides frontend role checking.
