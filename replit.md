...
*   **My Progress Page**: `/profile/progress` route showing user's sanitized wellness snapshot. Displays energy/stress/consistency metrics, switchboard status with 8 switch cards, weekly wins (completed actions count), and 14-day pattern summaries. Works for both authenticated and guest users via localStorage. User progress APIs: `/api/progress/summary`, `/api/progress/switches`, `/api/progress/patterns`, `/api/recommendation/today`.
